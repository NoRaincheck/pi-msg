...
func Register(instance, service, domain string, port int, text []string, ifaces []net.Interface) (*Server, error) {
	...
	// Guarantee the hostname is fully qualified: a hostname that already ends
	// with the domain (e.g. macOS "mbpro.local") skips the branch above and
	// would otherwise miss its trailing dot, making miekg/dns reject every
	// probe and announcement with ErrFqdn.
	entry.HostName = strings.TrimSuffix(entry.HostName, ".") + "."
...
func RegisterProxy(instance, service, domain string, port int, host string, ips []string, text []string, ifaces []net.Interface) (*Server, error) {
	...
	// Guarantee the hostname is fully qualified (see Register above).
	entry.HostName = strings.TrimSuffix(entry.HostName, ".") + "."
